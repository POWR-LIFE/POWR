 ...
 * within the wheel — past it they read as a ship's helm). Gold-on-dark,
 * parametric on size — small on the Rewards balance row, large as the
 * Vault screen centrepiece.
 */
export function VaultDoor({ size }: { size: number }) {
  ...
  return (
    <Svg width={size} height={size} viewBox="0 0 100 100">
      {/* Outer rim */}
      <Circle cx={50} cy={50} r={48} stroke="rgba(232,210,0,0.55)" strokeWidth={2.5} fill="rgba(20,20,20,0.92)" />
      {/* Bolts */}
      {bolts.map((b, i) => (
        <Circle key={i} cx={b.cx} cy={b.cy} r={1.8} fill="rgba(232,210,0,0.6)" />
      ))}
      ...
      <Circle cx={50} cy={50} r={18} stroke={GOLD} strokeWidth={2.5} fill="rgba(0,0,0,0.3)" />
      {spokes.map((s, i) => (
        <Line key={i} x1={s.x1} y1={s.y1} x2={s.x2} y2={s.y2} stroke={GOLD} strokeWidth={2} strokeLinecap="round" />
      ))}
      {/* Hub */}
      <Circle cx={50} cy={50} r={6} stroke={GOLD} strokeWidth={1.5} fill="rgba(20,20,20,1)" />
    </Svg>
  );
}
