...
> Status: **Phase 1 done 2026-08-28 · Post-event fixes done 2026-09-05 · Phase 2 items 7–9 done 2026-09-06, item 6 open · Phase 3 signal-driven** · Owner: Jamie · Last review: 2026-09-06
>
...
| --- | --- | --- | --- |
| 1 | **Raw geofence telemetry.** `geofence_region_events` is client-inserted, kept 30 days since 6 Sep (90 for the low-volume decision kinds). Re-measured 6 Sep: an iOS member writes ~20–80 rows/day, a real Android member 150–250 (nearly all `rearm_skipped`, tallied in the next OTA), and the two bench phones were 41% of all rows. | 63 k rows · 25 MB (6 Sep, before the retention change) | ~3–5 k active members; the diet roughly halves the per-member rate |
...
| 6 | **Partner geometry cache on the phone** (added 6 Sep). Every active partner location in one AsyncStorage key so headless detection needs no network. | 7,871 partners ≈ 1.4 MB, paged fetch, 24 h TTL; Android's AsyncStorage DB defaults to 6 MB | Partner growth, not member growth: ~25–30 k locations. Tile by region when partners pass ~15 k. |
| 7 | **Native region budget vs partner density** (added 6 Sep). The phone arms the nearest 19 (iOS) / 49 (Android) partners plus a sentinel of at least 2 km; when 19 partners sit inside 2 km of a member the sentinel can no longer see a walk to an un-armed gym and only the sweep catches it. | 19th-nearest partner to ONE LDN is 5.4 km; 4 partners within 2 km | Dense-city partner density. Signal: distance to the 19th-nearest partner at each member's home dropping under 2 km. |
...
| 7 | **RLS clean-up migration** | now | ✅ 2026-09-06 · PR #456 · migration `rls_initplan_and_hot_table_policies` | `(select auth.uid())` in place of `auth.uid()` (134 warnings) and consolidate the multiple permissive policies (237) on `point_transactions`, `gym_visits`, `gym_visit_events`, `geofence_region_events`, `activity_sessions`, `profiles`. Same access outcome; removes the per-row CPU multiplier before it matters. Verify with the advisor list before/after. **Shipped:** all 134 public policies rewritten (advisor now 0), the eight hot tables consolidated (advisor 237 → 177, 0 on hot tables), rolled-back self-test as member and admin. ⚠ One outcome changed on purpose: `profiles` had three UPDATE policies and the weakest let a member set their own `is_admin` / `is_pro` (the sync trigger then promoted them into `admin_roles`); the merged policy pins both columns. |
| 8 | **Housekeeping** (same migration as 7 or its own) | now | ✅ 2026-09-06 · PR #456 · migration `ops_housekeeping_retention_and_indexes` + dashboard | Drop duplicate index `referrals_referred_once` (≡ `referrals_referred_id_key`). Index `gym_visits.claimed_session_id` (claim-points step 11c looks up by it). Switch the auth server to percentage-based connections (else a compute upgrade never reaches sign-in). Fix `cron_silent` reporting a snapshot-system-health failure the run table doesn't hold. **Shipped:** both index changes; `cron.job_run_details` retention 30 → 7 days (91 MB → vacuumed); daily `VACUUM net._http_response` (43 MB of dead space for 395 KB of content, vacuumed). Auth connections switched to **Percentage (17%, = 10 of 60 today)** by Jamie in the dashboard (Authentication → Performance → Connection management) the same day. `cron_silent` untouched. |
| 9 | **Geofence telemetry diet** | now — needs a product call on how much per-visit history we keep | ✅ 2026-09-06 · PR #456 · server half live; client half in the next OTA | Ceiling #1. Options by effort: 30-day retention for rows of closed visits → client-side sampling of `sweep` / `wake_received` (7 k of this week's 11 k rows) → keep raw only while a visit is open, aggregate the rest. **Shipped:** region events kept 30 days (90 for the low-volume decision kinds: enter/exit/check-in/location/auth/stream failures); `fence_refresh` and presence-push rows kept 30 days; bench ping cadence 5 → 30 min (beacon v50). Client (OTA): `rearm_skipped` becomes one tallied row per half hour instead of one per refusal. Sweeps and wakes are deliberately NOT sampled — a `handoff` row with no `checked_in` after it is the freeze signature, and sampling would blind it. |
| 9a | **Fleet presence sweep ON** (from the 6 Sep review, not on the 27 Aug list) | now | ✅ 2026-09-06 · PR #456 · beacon v50 | `FLEET_INTERVAL_MIN` 0 → 240: every fresh-token device is woken every 4 h to ask "am I in a gym right now?" and to reconcile a zombie session against a real fix — entry and departure detection that need no fence. Pursuit (1/min for ≤ 8 min after a wake-ring ENTER that has not converted) is on for Android fleet-wide, bench-only on iOS because of Apple's background-push budget. First tick: 45 members pinged, 25 answered inside 5 min, both real Android members among them. Open visits are not pinged faster — the presence pass and the nudges already own them. |
| 9b | **Location-off close** | now | ✅ 2026-09-06 · PR #456 · `system_config.location_close_mode` = `on` | Staged in `observe` since 9 Aug; the five observe verdicts in 30 days were all plausible (`would_close` true, no reason flapping). Config only, reversible. |
