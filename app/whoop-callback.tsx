...
import { completeWhoopAuth } from '@/lib/health/providers/whoopProvider';
...
export default function WhoopCallback() {
    ...
    useEffect(() => {
        (async () => {
            ...
            if (error) {
                console.warn('[whoop-callback] OAuth error param:', error);
                setFailReason(`Whoop returned: ${error}`);
                setStatus('failed');
                return;
            }
            if (!code || !state) {
                console.warn('[whoop-callback] missing code/state', { code: !!code, state: !!state });
                setFailReason('Missing authorization code');
                setStatus('failed');
                return;
            }
            try {
                const ok = await completeWhoopAuth(code, state);
                if (!ok) {
                ...
                console.warn('[whoop-callback] completeWhoopAuth threw:', e);
                ...
                if (user) {
                    ...
                    const nextConns = {
                        ...
                        whoop: {
                            ...
                            scopes: ['read:recovery', 'read:cycles', 'read:sleep', 'read:workout', 'read:body_measurement', 'read:profile'],
                        },
                    };
                    await supabase.from('profiles').update({
                        health_provider_connections: nextConns,
                        active_health_provider: prof?.active_health_provider ?? 'whoop',
                    }).eq('id', user.id);
                }
            } catch {}
    ...
    return (
        <View style={{ flex: 1, backgroundColor: '#0d0d0d', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            ...
            <Text style={{ color: '#F2F2F2', marginTop: 16, fontSize: 14, textAlign: 'center' }}>
                {status === 'exchanging' && 'Connecting Whoop…'}
