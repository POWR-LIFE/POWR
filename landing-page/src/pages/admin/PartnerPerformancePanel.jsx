...
import {
    Activity, Users, Clock, BarChart3, MapPin,
    AlertTriangle, Shield, TrendingUp, Calendar, Star
} from 'lucide-react';
...
/**
 * Per-partner performance analytics. Self-contained: owns its time-range,
 * fetches sessions + member profiles for `partner.id`, and renders the full
 * leaderboard/session breakdown. Shared by the standalone GymDetail page
 * (/admin/performance/:partnerId) and the Performance tab on PartnerProfile.
 */
export default function PartnerPerformancePanel({ partner }) {
                            ...
                            <div className="divide-y divide-[#E6E6E1]">
                                {members.slice(0, 10).map((m, i) => (
                                    <Link
                                        ...
                                        className="flex items-center gap-6 px-10 py-6 group hover:bg-[#F4F4F1] transition-all"
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2">
                                                <span className="text-sm font-bold text-[#222222] truncate group-hover:text-[#8a7600] transition-colors">
                                ...
                                {pagedSessions.map(s => {
                                    ...
                                    return (
                                        <Link
                                            ...
                                            className={`grid grid-cols-[1fr_5rem_5rem_6rem_4rem] gap-4 items-center px-10 py-5 transition-all hover:bg-[#F4F4F1] group ${s.flagged ? 'bg-red-500/3 border-l-2 border-red-400' : ''}`}
                                        >
                                            {/* Member */}
