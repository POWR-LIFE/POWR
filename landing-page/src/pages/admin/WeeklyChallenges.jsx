...
function ActiveChallengeCard({ category, challenge, isOverridden, onScrollToCatalog, onClear }) {
	...
	return (
		<div className={`rounded-3xl border p-6 flex flex-col gap-4 transition-all ${
			isOverridden
				? 'border-[#FF5C00]/25 bg-[#FF5C00]/5'
				: 'border-[#E6E6E1] bg-white'
		}`}>
			<div className="flex items-start justify-between gap-3">
				<div className="flex items-center gap-3 min-w-0">
					<div className="w-10 h-10 rounded-2xl border border-[#E8D200]/20 bg-[#E8D200]/10 flex items-center justify-center shrink-0">
						<CatIcon category={category} />
					</div>
					<div className="min-w-0">
						<div className="text-[9px] uppercase tracking-[0.3em] text-[#999999] font-black">{meta?.label ?? category}</div>
						{isOverridden
							? <div className="text-[9px] uppercase tracking-[0.2em] text-[#FF5C00] font-black mt-0.5">Pinned override</div>
							: <div className="text-[9px] uppercase tracking-[0.2em] text-[#BBBBBB] font-black mt-0.5">Auto rotation</div>
						}
					</div>
				</div>
				<div className="flex items-center gap-2 shrink-0">
					...
					<button
						...
						className="h-8 px-4 rounded-full border border-[#E8D200]/20 bg-[#E8D200]/10 text-[#8a7600] text-[9px] font-black uppercase tracking-[0.25em] hover:bg-[#E8D200]/20 transition-colors"
					>
						Change
					</button>
				</div>
			...
			<div>
				<div className="flex items-center gap-3 mb-2 flex-wrap">
					<span className="text-xl font-light tracking-tight text-[#1A1A1A]">{challenge.title}</span>
					<TierBadge tier={challenge.tier} />
				</div>
				<p className="text-[#999999] text-sm leading-relaxed">{challenge.description}</p>
			</div>

			<div className="mt-auto flex items-center justify-between pt-3 border-t border-[#EFEFEC]">
				<span className="text-[9px] uppercase tracking-[0.35em] text-[#AAAAAA] font-black">Points reward</span>
				<span className="text-lg font-light text-[#8a7600]">+{challenge.points}</span>
			</div>
		</div>
	);
}
...
function CatalogCategorySection({ category, challenges, weekOverrides, autoChallenge, onPin, onUnpin }) {
	...
	return (
		<div className="rounded-3xl border border-[#E6E6E1] bg-white overflow-hidden">
			<button
				className="w-full flex items-center justify-between px-8 py-6 hover:bg-[#F4F4F1] transition-colors"
				onClick={() => setOpen((v) => !v)}
			>
				<div className="flex items-center gap-4">
					...
					<div className="text-left">
						<div className="text-base font-light text-[#1A1A1A]">{meta?.label ?? category}</div>
						<div className="text-[9px] uppercase tracking-[0.35em] text-[#AAAAAA] font-black mt-0.5">
			...
			{open && (
				<div className="border-t border-[#EFEFEC] divide-y divide-[#E6E6E1]">
					{challenges.map((c) => {
						const isAuto   = c.id === autoChallenge?.id && !activeOverrideId;
						const isPinned = c.id === activeOverrideId;
						const isHighlight = isAuto || isPinned;
						return (
							<div key={c.id} className={`px-8 py-5 flex items-center gap-6 ${isHighlight ? 'bg-[#F4F4F1]' : ''}`}>
								<div className="flex-1 min-w-0">
									<div className="flex items-center gap-3 mb-1 flex-wrap">
										<span className={`text-base font-light ${isHighlight ? 'text-[#1A1A1A]' : 'text-[#777777]'}`}>
											{c.title}
										</span>
										...
										{isPinned && (
											<span className="text-[9px] uppercase tracking-[0.3em] text-[#8a7600] font-black border border-[#E8D200]/20 bg-[#E8D200]/10 rounded-full px-3 py-1">
												Pinned
											</span>
										)}
									</div>
									<p className="text-[#999999] text-sm">{c.description}</p>
									<div className="mt-1 text-[9px] uppercase tracking-[0.3em] text-[#BBBBBB] font-black">+{c.points} pts</div>
								</div>
								...
								<div className="shrink-0">
									{isPinned ? (
										...
										<button
											...
											className="h-9 px-5 rounded-full border border-[#E6E6E1] bg-[#F4F4F1] text-[#999999] text-[9px] font-black uppercase tracking-[0.25em] flex items-center gap-2 hover:border-[#E8D200]/30 hover:text-[#8a7600] transition-colors"
										>
											<Flag size={11} /> Pin
										</button>
									)}
...
export default function WeeklyChallenges() {
	...
	const stats = [
		{ label: 'Catalog Size',     value: totalActive,           tone: '#8a7600', icon: Activity },
		{ label: 'Categories',       value: CATEGORY_ORDER.length, tone: '#10B981', icon: Check },
		{ label: 'Overrides Active', value: overrideCount,         tone: overrideCount > 0 ? '#FF5C00' : '#BBBBBB', icon: Flag },
	...
	return (
		<div className="px-4 lg:px-0 py-20 animate-in fade-in slide-in-from-bottom-8 duration-1000">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-12">
				{stats.map((item) => (
					<div key={item.label} className="rounded-3xl border border-[#E6E6E1] bg-white p-8">
						<div className="flex items-center justify-between mb-8">
							<div className="text-[10px] uppercase tracking-[0.4em] text-[#BBBBBB] font-black">{item.label}</div>
				...
				{[thisWeek, nextWeek].map((week) => (
					<button
						...
						className={`h-12 px-8 rounded-full text-[10px] font-black uppercase tracking-[0.3em] border transition-all ${
							selectedWeek === week
								? 'border-[#E8D200]/40 bg-[#E8D200]/10 text-[#8a7600]'
								: 'border-[#E6E6E1] bg-white text-[#999999] hover:text-[#666666]'
						}`}
					>
						{week === thisWeek ? 'This Week' : 'Next Week'}
						<span className={`ml-2 normal-case tracking-normal font-normal ${selectedWeek === week ? 'text-[#8a7600]' : 'text-[#BBBBBB]'}`}>
							{weekLabel(week)}
						</span>
					</button>
				))}
				{saving && (
					<span className="ml-4 text-[10px] uppercase tracking-[0.35em] text-[#999999] font-black flex items-center gap-2">
			...
			<div className="mb-6">
				<div className="flex items-center gap-3 mb-2">
					<span className="text-[10px] uppercase tracking-[0.5em] text-[#999999] font-black">Active This Week</span>
					{overrideCount > 0 && (
						<span className="text-[9px] uppercase tracking-[0.3em] text-[#FF5C00] font-black border border-[#FF5C00]/20 bg-[#FF5C00]/10 rounded-full px-3 py-1">
				...
				<p className="text-[11px] text-[#AAAAAA] mb-6">
					The 5 challenges shown on the app home screen for {selectedWeek} ({weekLabel(selectedWeek)}).
					Orange border = admin override. Use <strong className="text-[#1A1A1A]">Change</strong> to jump to that category in the catalog below.
				</p>
			</div>

			<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5 gap-5 mb-12">
			...
			{/* ── How it works ────────────────────────────────────────────── */}
			<div className="rounded-[2rem] border border-[#E6E6E1] bg-white p-10 mb-12">
				<h2 className="text-2xl font-light tracking-tight text-[#1A1A1A] mb-8">How the rotation works</h2>
				<div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-[#222222]">
					<div className="rounded-2xl border border-[#E6E6E1] bg-[#F4F4F1] px-6 py-5">
						<div className="text-[10px] uppercase tracking-[0.35em] text-[#BBBBBB] font-black mb-3">1. Auto rotation</div>
					...
					<div className="rounded-2xl border border-[#E6E6E1] bg-[#F4F4F1] px-6 py-5">
						<div className="text-[10px] uppercase tracking-[0.35em] text-[#BBBBBB] font-black mb-3">2. Pin an override</div>
						<div className="text-sm leading-relaxed">Use the <strong className="text-[#1A1A1A]">Pin</strong> button next to any catalog entry to force that challenge for a specific week and category. Saved to <code className="text-[#8a7600] text-xs">system_config</code>.</div>
					</div>
					<div className="rounded-2xl border border-[#E6E6E1] bg-[#F4F4F1] px-6 py-5">
						<div className="text-[10px] uppercase tracking-[0.35em] text-[#BBBBBB] font-black mb-3">3. Reset</div>
						<div className="text-sm leading-relaxed">Click <strong className="text-[#1A1A1A]">Unpin</strong> or the reset icon on any overridden card to return it to the auto rotation for that week.</div>
					</div>
				</div>
			</div>
			...
			{/* ── Full catalog ─────────────────────────────────────────────── */}
			<div className="mb-6">
				<span className="text-[10px] uppercase tracking-[0.5em] text-[#999999] font-black">Full Catalog — {totalActive} Challenges</span>
				<p className="text-[11px] text-[#AAAAAA] mt-2">Expand a category to see all challenges and pin one for {selectedWeek}.</p>
			</div>
			...
			{Object.keys(allOverrides).filter((w) => w !== thisWeek && w !== nextWeek && Object.keys(allOverrides[w] ?? {}).length > 0).length > 0 && (
				<div className="mt-16">
					<span className="text-[10px] uppercase tracking-[0.5em] text-[#999999] font-black">Past Overrides</span>
					<div className="mt-6 rounded-3xl border border-[#E6E6E1] bg-white overflow-hidden">
								...
								.map(([week, ov]) => (
									<div key={week} className="px-8 py-5 flex items-center gap-6">
										<div className="w-32 shrink-0">
											<div className="text-sm text-[#999999]">{week}</div>
											<div className="text-[10px] text-[#BBBBBB]">{weekLabel(week)}</div>
										</div>
										<div className="flex flex-wrap gap-2 flex-1">
											{Object.entries(ov).map(([cat, id]) => {
												const c = CATALOG.find((x) => x.id === id);
												return (
													<span key={cat} className="inline-flex items-center gap-2 border border-[#E6E6E1] rounded-full px-4 py-1.5 text-[10px] text-[#999999]">
										...
										<button
											...
											className="shrink-0 w-8 h-8 rounded-full border border-[#E6E6E1] text-[#AAAAAA] flex items-center justify-center hover:text-[#F43F5E] hover:border-[#F43F5E]/30 transition-colors"
										>
											<X size={13} />
										</button>
									</div>
