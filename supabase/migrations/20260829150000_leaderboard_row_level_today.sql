-- =============================================================
-- Leaderboard rows: level basis + points earned today
--
-- Every board row (league views + live event board) now carries:
--   * total_earned — the LEVEL basis. profiles.level is a dead column
--     (always 1); the real basis is positive ledger sum + PENDING vault,
--     the exact get_my_points_summary.total_earned formula.
--   * today_points — ledger earn/adjustment/penalty since UTC midnight.
--     UTC day to match the weekly view's UTC week. The client hides the
--     chip at 0, so it reads as an "active today" marker.
--
-- The league views are security_invoker: point_transactions /
-- vault_deposits RLS is self-or-admin, so a member sees other rows as 0
-- there. Pre-existing (weekly_points has the same shape) and dormant —
-- the League tab runs the event board (LEAGUE_LIVE=false). The event
-- board RPC is security definer and reads everyone.
-- =============================================================

-- ── 1. Shared row extras (called from definer RPCs only) ─────────────────────

create or replace function public._leaderboard_row_extras(p_user_id uuid)
  ...
  select
    (
      coalesce((select sum(pt.amount) from public.point_transactions pt
                 where pt.user_id = p_user_id and pt.amount > 0), 0)
      + coalesce((select sum(vd.amount) from public.vault_deposits vd
                   where vd.user_id = p_user_id and vd.released_at is null), 0)
    )::int as total_earned,
    coalesce((select sum(pt.amount) from public.point_transactions pt
               where pt.user_id = p_user_id
                 and pt.type in ('earn', 'adjustment', 'penalty')
                 and pt.created_at >= date_trunc('day', now() at time zone 'utc') at time zone 'utc'), 0)::int
      as today_points;
$$;

revoke execute on function public._leaderboard_row_extras(uuid) from public, anon, authenticated;
grant execute on function public._leaderboard_row_extras(uuid) to service_role;

-- ── 2. League views — append total_earned + today_points ─────────────────────
-- CREATE OR REPLACE VIEW may only append columns, so the existing seven stay
-- in place (level included, dead as it is). The type filter moves from the
-- join to FILTER clauses so one scan feeds every column.
...
CREATE OR REPLACE VIEW public.leaderboard_weekly WITH (security_invoker = true) AS
  SELECT
    ...
    (
      COALESCE(SUM(pt.amount) FILTER (WHERE pt.amount > 0), 0)
      + COALESCE((SELECT SUM(vd.amount) FROM public.vault_deposits vd
                   WHERE vd.user_id = p.id AND vd.released_at IS NULL), 0)
    )::INT AS total_earned,
    COALESCE(SUM(pt.amount) FILTER (
      WHERE pt.type IN ('earn', 'adjustment', 'penalty')
...
-- ── 3. get_event_leaderboard — real rows carry the extras ────────────────────
-- Body identical to 20260826200000 except the two real-row builders (live
-- standings + revealed/settled results). Admin preview theatre rows stay
-- bare — the client hides the pill/chip when the keys are absent.
...
begin
  ...
  if v_event.status in ('revealed', 'settled') then
    select jsonb_agg(jsonb_build_object(
      ...
      cross join lateral public._leaderboard_row_extras(r.user_id) x
