-- =============================================================
-- admin_get_users() v4
-- The Users page renders a watch icon on every provider chip, so
-- "Apple Health" reads as "has an Apple Watch" when it only means
-- "granted HealthKit permission on an iPhone". The client already
-- stamps per-sample device provenance into
-- health_snapshots.source_detail ("Apple Watch", "iPhone",
-- "Garmin", "iPhone, Apple Watch", ...), so return the distinct
-- device tokens per user and let the page tell watch owners from
-- phone-only users honestly.
