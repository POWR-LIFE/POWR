-- A confirm must not stamp proof onto a FINISHED visit (2026-08-13 PM).
--
-- Field capture: after the walk-out, the Android client's exit flush replayed a
-- stale cached fix (27 m / 30 m acc, frozen from mid-visit) against the already
-- closed visit 9a5e3c55 — and the stamp update happily wrote last_proven_at /
-- last_confirmed_at 67 minutes AFTER ended_at, on a row the reaper had closed.
-- Credit was never at risk (both credit branches gate on status open/claimed),
-- but the proof columns are forensic ground truth — last_proven_at after
-- ended_at poisons every later read of what actually happened.
--
-- Only the stamp gains the guard (`and ended_at is null`). The gym_visit_events
-- insert stays unconditional — "a wake reached JS" is true and worth recording
-- whether or not the visit is still live. confirm_gym_visit_v3 delegates here;
-- the legacy v1 predates the proof columns' current semantics and is left
-- untouched.
...
begin
  ...
  -- ended_at is null: proof columns describe a LIVE visit only (see header).
  update gym_visits
