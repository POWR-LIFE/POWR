-- =============================================================
-- REWARD PLACEMENTS → brand history + cooldown / lifetime caps
-- =============================================================
--  audience_history: 'new' (never redeemed this brand — acquisition) or
--    'returning' (has redeemed this brand — loyalty). Brand = rewards.brand_name
--    (falls back to partner_id when brand_name is null). Refunded redemptions
--    don't count.
--  cooldown_hours: don't re-surface to a user within N hours of the last time.
--  max_impressions_per_user_total: lifetime surfacing cap per user.
-- =============================================================

alter table public.reward_placements
  add column if not exists audience_history text not null default 'any'
    check (audience_history in ('any', 'new', 'returning')),
  add column if not exists cooldown_hours integer,
  add column if not exists max_impressions_per_user_total integer;
    ...
    and (
      ...
      or (
        ...
        where e.placement_id = c.id and e.user_id = auth.uid()
          and e.event_type = 'surfaced' and e.created_at > now() - interval '1 day'
