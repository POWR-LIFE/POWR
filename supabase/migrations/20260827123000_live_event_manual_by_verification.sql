-- Live events: "manual" means verification = 'manual', not source = 'manual_log'.
--
-- 2026-08-27, FNL x POWR: before flipping count_manual off to level the board,
-- the predicate was checked against the ledger. The native HealthKit / Health
-- Connect workout sync writes through logManualSession, so every one of those
-- rows carries source = 'manual_log' while its verification is 'health' or
-- 'wearable' (207 rows / 1,331 pts in the last 30 days, vs ONE genuinely manual
-- row). With the old predicate, count_manual = false would have removed every
-- Apple Health user's workouts from the board — the opposite of what the toggle
-- means. The session's verification column is the only truthful "was this
-- self-reported" signal, so the source check is dropped.
--
