-- COSMETIC / WRITER PARITY ONLY. The presence and credit gating below is byte-identical
-- to 20260810140000_proven_requires_fresh_fix.sql — v_proven, the 120 s fix-age mirror,
-- the two-clock v_elapsed_min agreement, the same-UTC-day session lookup and every
-- threshold are unchanged. Read no policy change into this migration.
--
-- The one change is inside the activity_sessions insert's raw_gps: it now carries
-- 'partnerName' beside 'partnerId'.
--
-- Why: the two gym-session writers have always disagreed about raw_gps. The client
-- (context/GeofenceContext.tsx) stamps { partnerId, partnerName, entryTimestamp }; this
-- RPC has stamped { partnerId, entryTimestamp, createdBy } since its first version on
-- 2026-07-14, and the client's 23505 fallback patches only ended_at/duration_sec, so a
-- row created here never acquired the name afterwards. Anything reading the snapshot
-- rather than the partner_id column therefore showed a venue for foreground claims and
-- nothing for background/APNs-wake claims by the same athlete at the same gym — 19 such
-- rows in prod as of today. The admin Activity Logs did exactly that.
--
-- The admin panel now resolves venues from the partner_id column (which BOTH writers have
-- always set), so this does not fix any live surface on its own — historical rows are
-- already covered there. It exists so the two writers stop diverging and so the snapshot
-- means the same thing whoever wrote it.
--
-- A deleted partner yields 'partnerName': null; every reader treats that as absent and
-- falls back to partner_id, so the subselect cannot fail the insert.
