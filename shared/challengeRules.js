/**
 * Weekly-challenge CORE — rule engine + catalog + rotation. PURE, dependency-free (CommonJS).
 *
 * Single source of truth for the new (auto-detected, multi-challenge) system.
 * Consumed by:
 *   - the app via shared/weeklyChallenges.js (ESM wrapper that re-exports this)
 *   - the unit test scripts/test-challenge-rules.js (require)
 * A mirror copy lives at supabase/functions/_shared/challenges.ts (Deno ESM) — keep in sync.
 *
 * Must stay free of React Native / Deno / Supabase imports.
 */
...
const CATEGORY_ORDER = ['gym', 'walking', 'running', 'cycling', 'multi'];
...
function getChallengeById(id, catalog = CATALOG) {
  return catalog.find((c) => c.id === id) || null;
}

// ── Rotation + week helpers ───────────────────────────────────────────────────
...
function weekNumber(isoWeek) {
  const m = /^(\d+)-W(\d+)$/.exec(isoWeek || '');
  if (!m) return 0;
  return parseInt(m[1], 10) * 53 + parseInt(m[2], 10);
}

/** One supported challenge per category for the given ISO week, advancing weekly. */
function getActiveChallengesForWeek(isoWeek, catalog = CATALOG) {
...
/** Validate + normalize an admin-provided NEW-shape catalog; falls back to bundled. */
function parseChallengeCatalog(value) {
...
function computeExpiresIn(expiresAt) {
  if (!expiresAt) return '';
...
function computeUrgency(expiresAt) {
...
/** Local date components from a UTC timestamp + the user's offset (minutes). */
function localParts(startedAtISO, utcOffsetMinutes) {
  const localMs = new Date(startedAtISO).getTime() + utcOffsetMinutes * 60 * 1000;
  const d = new Date(localMs);
  const dateKey = `${d.getUTCFullYear()}-${String(d.getUTCMonth() + 1).padStart(2, '0')}-${String(d.getUTCDate()).padStart(2, '0')}`;
  return { dateKey, dow: (d.getUTCDay() + 6) % 7, hour: d.getUTCHours() };
}

function dateKeyToDayNum(key) {
  const [y, m, d] = key.split('-').map(Number);
  return Math.floor(Date.UTC(y, m - 1, d) / 86400000);
}

/**
 * Build the normalized evaluation context from raw activity_sessions.
 * Excludes 'manual' (no sensor backing) and 'sleep' (not an activity).
 */
function buildContext(rawSessions, utcOffsetMinutes, stepWindowRows = []) {
  const sessions = [];
  ...
  const stepWindows = new Map();
  for (const w of stepWindowRows || []) {
    stepWindows.set(w.date, {
      morning: w.before_9am || 0,
      midday: w.midday_12_14 || 0,
      evening: w.after_6pm || 0,
    });
  }
...
function sessionMatches(s, rule) {
...
const clampResult = (progress, target) => ({ progress, target, met: progress >= target });

/** Evaluate a challenge rule against a normalized context. */
function evaluateChallenge(rule, ctx) {
  switch (rule.kind) {
    case 'session_count': {
      const n = ctx.sessions.filter((s) => sessionMatches(s, rule)).length;
      return clampResult(n, rule.target);
    }
    case 'distinct_days': {
      const days = new Set();
      ...
      return clampResult(days.size, rule.target);
    }
    case 'daily_metric_days': {
      ...
      return clampResult(count, rule.target);
    }
    case 'weekly_sum': {
      let sum = 0;
      if (rule.metric === 'steps') {
        for (const { steps } of ctx.dailySteps.values()) sum += steps;
      } else {
        for (const s of ctx.sessions) if (s.category === rule.category) sum += s.distance_m;
      }
      return clampResult(sum, rule.target);
    }
    case 'weekend_sum': {
      let sum = 0;
    ...
    case 'count_with_min_metric': {
      ...
      const n = ctx.sessions.filter((s) => s.category === rule.category && (s[metric] || 0) >= rule.threshold).length;
      return clampResult(n, rule.target);
    }
    case 'count_and_sum': {
      const metric = rule.metric || 'distance_m';
      ...
      const sum = matching.reduce((a, s) => a + (s[metric] || 0), 0);
      return { progress: count, target: rule.target, met: count >= rule.target && sum >= rule.min };
    }
    case 'distinct_categories': {
      const counts = {};
      for (const s of ctx.sessions) {
        if (!MAIN_CATEGORIES.includes(s.category)) continue;
        counts[s.category] = (counts[s.category] || 0) + 1;
      }
      const perCat = rule.perCat || 1;
      const qualifying = Object.values(counts).filter((c) => c >= perCat).length;
      return clampResult(qualifying, rule.target);
    }
    case 'same_day_combo': {
      const byDay = new Map();
      for (const s of ctx.sessions) {
      ...
      for (const set of byDay.values()) {
        if (set.has(rule.a) && rule.b.some((b) => set.has(b))) days++;
      }
      return clampResult(days, rule.target);
    }
    case 'spaced_days': {
      const nums = [
        ...new Set(ctx.sessions.filter((s) => s.category === rule.category).map((s) => s.dateKey)),
      ]
        .map(dateKeyToDayNum)
        .sort((a, b) => a - b);
      let count = 0;
    ...
    case 'step_window': {
      let count = 0;
      for (const w of ctx.stepWindows.values()) {
        if ((w[rule.window] || 0) >= rule.threshold) count++;
      }
      return clampResult(count, rule.target);
    }
    default:
